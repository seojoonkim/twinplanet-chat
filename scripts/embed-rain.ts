/**
 * Rain(비) 아이돌 임베딩 스크립트
 * 실행: SUPABASE_URL=... SUPABASE_ANON_KEY=... OPENAI_API_KEY=... npx tsx scripts/embed-rain.ts
 */
...
// 기존 Rain 데이터 삭제
async function deleteExistingRainData(): Promise<void> {
  console.log('🗑️  Deleting existing Rain data...');
  
  const response = await fetch(
    `${SUPABASE_URL}/rest/v1/idol_knowledge?idol_id=eq.rain`,
    {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
...
// 메인 실행
async function main() {
  console.log('🚀 Starting Rain embedding...\n');

  const rainDir = path.join(process.cwd(), 'public/idols/rain');
  const mdFiles = fs.readdirSync(rainDir).filter(f => f.endsWith('.md'));

  console.log(`📁 Found ${mdFiles.length} MD files in rain folder\n`);

  // 기존 데이터 삭제
  await deleteExistingRainData();

  const allItems: KnowledgeItem[] = [];

  // 파일별로 청크 생성
  for (const filename of mdFiles) {
    const filePath = path.join(rainDir, filename);
